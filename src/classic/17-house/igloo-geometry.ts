...
const iglooGeometry = new BufferGeometry()
iglooGeometry.setAttribute("position", new BufferAttribute(pospoints, 3, false))
iglooGeometry.setAttribute("normal", new BufferAttribute(normpoints, 3, false))
iglooGeometry.setAttribute("uv", new BufferAttribute(uvpoints, 3, false))

type Mapper<T, R> = (arr: T) => R
let set  = new Set(tpoints.map(JSON.stringify as Mapper<TPoint, string>))
iglooIndices = Array.from(set).map(JSON.parse as Mapper<string, TPoint>)

export { iglooGeometry, iglooIndices }